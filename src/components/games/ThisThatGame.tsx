...
const ThisThatGame = ({
  ...
  const handleSelect = async (side: 'top' | 'bottom') => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.functions.invoke('record-response', {
        body: {
          game_type: 'thisthat',
  ...
  return <div className="flex flex-col items-center">
      ...
      }} onClick={() => handleSelect('top')} className="w-full bg-card rounded-lg overflow-hidden border border-border cursor-pointer transition-all">
          <div className="aspect-square overflow-hidden">
            <img src={left.image} alt={left.title} className="w-full h-full object-cover" />
          </div>
        </motion.div>
        
        {/* VS Badge */}
        <div className="relative -my-6 z-10">
          <div className="w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg" style={{
      ...
      }} onClick={() => handleSelect('bottom')} className="w-full bg-card rounded-lg overflow-hidden border border-border cursor-pointer transition-all">
          <div className="aspect-square overflow-hidden">
            <img src={right.image} alt={right.title} className="w-full h-full object-cover" />
          </div>
        </motion.div>
      </div>
    </div>;
